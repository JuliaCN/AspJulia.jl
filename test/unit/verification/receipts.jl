@testset "verification receipt review enforces required evidence" begin
    ...
    template_reviews = review_asp_julia_verification_receipts(
        index,
    ...
    reviews = review_asp_julia_verification_receipts(index, [good_receipt])
    ...
    accepted_reviews = assert_asp_julia_verification_receipts_accepted(index, [good_receipt])
    ...
    mismatched_review = only(review_asp_julia_verification_receipts(index, [mismatched_binding]))
    mismatched_rendered = render_asp_julia_verification_receipt_reviews(
        [mismatched_review];
        project_root=root,
    )
    ...
    bad_reviews = review_asp_julia_verification_receipts(index, [bad_receipt])
    bad_rendered = render_asp_julia_verification_receipt_reviews(bad_reviews; project_root=root)
    bad_json = render_asp_julia_verification_receipt_reviews_json(bad_reviews)
    ...
    @test_throws ErrorException assert_asp_julia_verification_receipts_accepted(
        index,
        [bad_receipt],
    )
    ...
    weak_waiver_review = only(review_asp_julia_verification_receipts(index, [weak_waiver]))
    ...
    concrete_waiver_review = only(review_asp_julia_verification_receipts(index, [concrete_waiver]))
    @test concrete_waiver_review.status == :waived

    orphan_reviews = review_asp_julia_verification_receipts(
        index,
